module RubygemsHelper
  ...
  def subscribe_link(rubygem)
    if signed_in?
      ...
      link_to t('rubygems.show.links.subscribe'), rubygem_subscription_path(rubygem),
        class: ['toggler', 'gem__link', 't-list__item'], id: 'subscribe',
        method: :post, remote: true, style: style
    else
      link_to t('rubygems.show.links.subscribe'), sign_in_path,
        class: [:toggler, 'gem__link', 't-list__item'], id: :subscribe
    end
  end
  ...
  def unsubscribe_link(rubygem)
    ...
    link_to t('rubygems.show.links.unsubscribe'), rubygem_subscription_path(rubygem),
      class: [:toggler, 'gem__link', 't-list__item'], id: 'unsubscribe',
      method: :delete, remote: true, style: style
  end

  def atom_link(rubygem)
    link_to t('rubygems.show.links.rss'),
      rubygem_versions_path(rubygem, format: 'atom'),
      class: 'gem__link t-list__item', id: :rss
  end

  def download_link(version)
    link_to t('rubygems.show.links.download'),
      "/downloads/#{version.full_name}.gem",
  ...
  def report_abuse_link(rubygem)
    ...
    report_abuse_url = 'http://help.rubygems.org/discussion/' \
      "new?discussion[private]=1&discussion[title]=#{encoded_title}"
    link_to t('rubygems.show.links.report_abuse'),
      report_abuse_url.html_safe,
