...
class UploadInfoFileJob < ApplicationJob
  ...
  def perform(rubygem_name:)
    ...
    upload_v2_info_file(gem_info, rubygem_name)
  end
  ...
  def upload_v2_info_file(gem_info, rubygem_name)
    compact_index_info = gem_info.compact_index_info_v2
    response_body = CompactIndex.info(compact_index_info)
    ...
    response = RubygemFs.compact_index.store(
      "v2/info/#{rubygem_name}", response_body,
      public_acl: false, # the compact-index bucket does not have ACLs enabled
      metadata: {
        "surrogate-control" => "max-age=3600, stale-while-revalidate=1800",
    ...
    logger.info(message: "Uploading v2 info file for #{rubygem_name} succeeded", response:)
    FastlyPurgeJob.perform_later(key: "s3-v2/info/#{rubygem_name}", soft: true)
  end
end
