...
$(function () {
  var getDumpData = function (target, type) {
    return $.get(
      "https://s3-us-west-2.amazonaws.com/rubygems-dumps/?prefix=production/public_" +
        type
    )
      .done(function (data) {
        var files, xml;
  ...
  var renderDumpList = function (files) {
    ...
    jQuery.each(files, function (idx, item) {
      if ("STANDARD" === item.StorageClass) {
        return content.push(
          "<li><a href='https://s3-us-west-2.amazonaws.com/rubygems-dumps/" +
            item.Key +
            "'>" +
            item.LastModified.replace(".000Z", "") +
            " (" +
