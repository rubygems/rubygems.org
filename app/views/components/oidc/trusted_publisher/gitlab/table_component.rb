...
class OIDC::TrustedPublisher::GitLab::TableComponent < ApplicationComponent
  ...
  def view_template
    dl(class: "tw-flex tw-flex-col sm:tw-grid sm:tw-grid-cols-2 tw-items-baseline tw-gap-4 full-width overflow-wrap") do
      dt(class: "description__heading") { t("oidc.trusted_publisher.gitlab.table.project_path") }
      dd { code { gitlab.project_path } }

      dt(class: "description__heading") { t("oidc.trusted_publisher.gitlab.table.ci_config_path") }
      dd { code { gitlab.ci_config_path } }

      if gitlab.environment.present?
        dt(class: "description__heading") { t("oidc.trusted_publisher.gitlab.table.environment") }
        dd { code { gitlab.environment } }
      end

      if gitlab.ref_type.present?
        dt(class: "description__heading") { t("oidc.trusted_publisher.gitlab.table.ref_type") }
        dd { code { gitlab.ref_type } }
      end

      if gitlab.branch_name.present?
        dt(class: "description__heading") { t("oidc.trusted_publisher.gitlab.table.branch_name") }
