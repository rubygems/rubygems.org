class Api::V1::OIDC::ApiKeyRolesController < Api::BaseController
  ...
  def show
    render json: @api_key.user.oidc_api_key_roles.find_by!(token: params_fetch(:token))
  end
  ...
  def set_api_key_role
    @api_key_role = OIDC::ApiKeyRole.active.find_by!(token: params_fetch(:token))
  end

  def decode_jwt
    raise UnverifiedJWT, "Provider missing JWKS" if @api_key_role.provider.jwks.blank?
    @jwt = JSON::JWT.decode_compact_serialized(params_fetch(:jwt), @api_key_role.provider.jwks)
