document.addEventListener('click', function (event) {
  ...
  try {
    ...
    fetch(url, { method: 'GET' })
      ...
      .catch(() => {
        const error_message = "<ul class='deps_item--error'>Request failed. please reload the page and try again</ul>"
        const container = trigger.parentElement.nextElementSibling.nextElementSibling
...
function arrow_toggler(current) {
  const toggler = "<span class='deps_expanded arrow_toggle deps_expanded-down'></span>"
...
function scope_display(current, deps, scope) {
  if (deps.length !== 0) {
    ...
    if (scope === "development") { new_gems = new_gems.nextElementSibling }
...
document.addEventListener('click', function (event) {
  ...
  if (el.nextElementSibling) el.nextElementSibling.classList.toggle('deps_toggle')
})
...
document.addEventListener('click', function (event) {
  ...
  if (runtime_div) {
    ...
    if (runtime_div.nextElementSibling) runtime_div.nextElementSibling.classList.toggle('deps_toggle')
  }
