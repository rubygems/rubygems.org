  ...
  def self.update?
    ...
    if !Dir.exist?('tmp/advisories')
      ...
      if system "git clone  --depth 1 http://github.com/rubysec/ruby-advisory-db #{Dir.pwd}/ruby-advisory-db"
        File.write('advisory_db_head_sha', new_sha) # TODO: store new sha after updating db
      end
    else
      Dir.chdir(Rails.root.join('tmp', 'advisories'))
  ...
  def self.mark_versions
    ...
    ActiveRecord::Base.transaction do
      Dir.glob("gems/**/*.yml") do |advisory_file|
        ...
        rubygem = Rubygem.find_by(name: advisory['gem'])
        unless rubygem.nil?
          ...
          all_versions.where(number: vulnerable_versions).each do |vuln_version|
            next if vuln_version.advisories.include? Advisory.where(cve: advisory['cve'])
            vuln_version.advisories.create(url: advisory['url'], cve: advisory['cve'], title: advisory['title'],
