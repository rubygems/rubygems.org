...
class RubygemsControllerTest < ActionController::TestCase
  ...
  context "On GET to show for a gem with both runtime and development dependencies" do
    ...
    should "show proper links to dependencies" do
      assert page.has_link?(@runtime.rubygem.name, href: "/gems/#{@runtime.rubygem.slug}")
      assert page.has_link?(@development.rubygem.name, href: "/gems/#{@development.rubygem.slug}")
    end
  end
