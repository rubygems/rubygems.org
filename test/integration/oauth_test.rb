...
class OAuthTest < ActionDispatch::IntegrationTest
  ...
  test "sets auth cookie when successful" do
    @octokit_stubs.get('/user/memberships/orgs/rubygems') { |env| [200, {'Content-Type' => 'application/json'}, JSON.generate(state: :active)]}
    @octokit_stubs.get('/orgs/rubygems/teams/rubygems-org/memberships/jackson-keeling') { |env| [200, {'Content-Type' => 'application/json'}, JSON.generate(state: :active)]}
  ...
  test "fails when user is not a member of the rubygems org" do
    @octokit_stubs.get('/user/memberships/orgs/rubygems') { |env| [403, {'Content-Type' => 'application/json'}, JSON.generate({message: "bad"})]}
  ...
  test "fails when user is not a member of the rubygems-org team" do
    @octokit_stubs.get('/user/memberships/orgs/rubygems') { |env| [200, {'Content-Type' => 'application/json'}, JSON.generate(state: :active)]}
    @octokit_stubs.get('/orgs/rubygems/teams/rubygems-org/memberships/jackson-keeling') { |env| [400, {'Content-Type' => 'application/json'}, JSON.generate(message: 'bad')]}
