...
class GemInfoTest < ActiveSupport::TestCase
  ...
  context ".compact_index_public_versions_v2" do
    setup do
      @ts               = 5.minutes.ago
    ...
    should "not return version updated after ts" do
      versions = GemInfo.compact_index_public_versions_v2(@ts)
    ...
    should "not fall back to info_checksum_v2 for yanked rows missing yanked_info_checksum_v2" do
      rubygem = create(:rubygem, name: "bar")
      indexed_version = create(:version, rubygem: rubygem, number: "1.0.0", created_at: 10.minutes.ago, info_checksum_v2: "v2qw2dwe")
      create(:version, :yanked, rubygem: rubygem, number: "2.0.0", created_at: 9.minutes.ago,
      ...
      assert_includes versions, CompactIndex::Gem.new("bar", [CompactIndex::GemVersion.new("1.0.0", "ruby", indexed_version.sha256, nil)])
    end
  end
end
