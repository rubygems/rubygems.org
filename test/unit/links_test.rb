...
class LinksTest < ActiveSupport::TestCase
  ...
  context "metadata includes non allowlisted uri key" do
    setup do
      metadata = {
        ...
        "non_allowlisted_uri" => "https://example.com"
      }
    ...
    should "create method for allowlisted keys" do
      allowlisted_keys = Links::LINKS.values.reject! { |k| k == "download_uri" }
      allowlisted_keys.each do |key|
        assert_equal "https://example.com", @links.send(key), "value doesn't match for method: #{key}"
      end
    end

    should "not create method for non allowlisted key" do
      refute_respond_to @links, "non_allowlisted_uri"
    end
  end
end
