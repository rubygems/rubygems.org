...
class EmailConfirmationTest < ApplicationSystemTestCase
  ...
  test "requesting confirmation mail with email of existing user" do
    ...
    assert_text "Sign in"
    assert has_selector? "#flash_notice", text: "Your email address has been verified"
  end
  ...
  test "re-using confirmation link, asks user to double check the link" do
    ...
    assert_text "Sign in"
    assert has_selector? "#flash_alert", text: "Please double check the URL or try submitting it again."
  end
