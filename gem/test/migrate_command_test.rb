...
class MigrateCommandTest < CommandTest
  ...
  context "ask about the gem" do
    ...
    should "find gem info if it exists" do
      FakeWeb.register_uri :get, "https://gemcutter.org/api/v1/gems/#{@name}.json", :body => @json
      @command.find(@name)
      assert_equal JSON.parse(@json), @command.rubygem
    end

    should "dump out if the gem couldn't be found" do
      FakeWeb.register_uri :get, "https://gemcutter.org/api/v1/gems/#{@name}.json", :body => "Not hosted here.", :status => 404
      @command.find(@name)
    ...
    should "dump out if bad json is returned" do
      FakeWeb.register_uri :get, "https://gemcutter.org/api/v1/gems/#{@name}.json", :body => "bad data is bad"
      @command.find(@name)
      assert_received(@command) { |subject| subject.say(anything) }
      assert_received(@command) { |subject| subject.terminate_interaction }
    end
  ...
  context "getting the token" do
    ...
    should "ask gemcutter to start the migration" do
      token = "SECRET TOKEN"
      FakeWeb.register_uri :post, "https://gemcutter.org/api/v1/gems/#{@name}/migrate", :body => token
      assert_equal token, @command.get_token
    end

    should "dump out if gem could not be found" do
      FakeWeb.register_uri :post, "https://gemcutter.org/api/v1/gems/#{@name}/migrate", :status => 404, :body => "not found"
      @command.get_token
    ...
    should "dump out if migration has already been completed" do
      FakeWeb.register_uri :post, "https://gemcutter.org/api/v1/gems/#{@name}/migrate", :status => 403, :body => "already migrated"
      @command.get_token
  ...
  context "checking if the rubygem was approved" do
    ...
    should "let the server decide the status" do
      FakeWeb.register_uri :put, "https://gemcutter.org/api/v1/gems/#{@name}/migrate", :body => "Success!", :status => 400
